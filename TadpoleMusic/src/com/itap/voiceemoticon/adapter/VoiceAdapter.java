...
public class VoiceAdapter extends ArrayListAdapter<Voice> {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (view == null) {
			view = getLayoutInflater().inflate(R.layout.list_item_hot_voice,
					null);
			viewHolder = new ViewHolder();
			viewHolder.textViewTitle = (TextView) view
					.findViewById(R.id.text_view_title);
			viewHolder.textViewTags = (TextView) view
					.findViewById(R.id.text_view_tags);
			viewHolder.btnShare = view.findViewById(R.id.btn_share);
			viewHolder.btnCollect = (ImageButton) view
		...
		if (VEApplication.isCollected(mContext, item.url)) {
			viewHolder.btnCollect
					.setImageResource(R.drawable.btn_hasfavorite_default);
		} else {
			viewHolder.btnCollect
					.setImageResource(R.drawable.btn_favorite_default);
		}
