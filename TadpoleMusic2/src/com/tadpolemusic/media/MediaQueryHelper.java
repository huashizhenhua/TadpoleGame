...
public class MediaQueryHelper {
     ...
     * @param context
     */
    public MediaQueryHelper(Context context) {
        mContext = context;
    }
    ...
    public ArrayList<MusicData> getLocalMusicDatas() {
        ArrayList<LocalMusicItem> itemList = getLocalMusicList();
        ArrayList<MusicData> retList = new ArrayList<MusicData>(itemList.size());
        for (int i = 0, N = itemList.size(); i < N; i++) {
            LocalMusicItem li = itemList.get(i);
            
            MusicData md = new MusicData();
            
            md.musicPath = li.getmFilePath();
            md.musicName = li.getFileTitle();
            md.musicDuration = li.getDuration();
            md.musicAritst = li.getSinger();
